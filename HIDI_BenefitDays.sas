/*
Purpose:
To calculate days on benefit from admin and reported sources.
*/
...
proc sql;
	create table HES_Individual as
	select a.*, 
	coalesce(b.Possible, a.snz_uid) as snz_uid_update, 
	coalesce(0*b.Possible+1,0) as geo_impute
	from HES_Individual a LEFT JOIN intdata.HES_Geo_Impute b
	on a.snz_uid = b.snz_uid
	order by snz_uid;
quit;
...
data HES_Individual;
	set HES_Individual;
	if geo_impute = 1 then snz_uid = snz_uid_update;
	if geo_impute = 1 then snz_spine_ind = 1;
run;
...
proc sql;
	...
	coalesce(b.Possible, a.snz_uid) as snz_uid_update, 
	coalesce(0*b.Possible+1,0) as geo_impute
	from HES_Income_Bens a LEFT JOIN intdata.HES_Geo_Impute b
	on a.snz_uid = b.snz_uid
	order by snz_uid;
quit;

data HES_Income_Bens;
	set HES_Income_Bens;
	if geo_impute = 1 then snz_uid = snz_uid_update;
run;
...
proc sql;
	...
	select distinct snz_uid, sum(CASE WHEN b.inc_source = '07' THEN b.hes_days END) as HES_UB_DAYS,
		...
		sum(CASE WHEN b.inc_source = '10' THEN b.hes_days END) as HES_IB_DAYS, 
...
additional_service_data=msd_spel_add_servf_code;
	...
	else if  (servf in ('115', '610', '611', '030', '330', '366', '666'))
	...
	else if  (servf in ('607', '608'))
        or (servf in ('675') and additional_service_data in ('FTJS3', 'FTJS4'))
...
data HES_Individual_Summary_14_15;
	...
	* subset data to spine and 1415 ;
	if snz_spine_ind = 0 or hes_per_age_nbr < 15 then DELETE;
	if hes_per_hes_year_code ne '1415' then DELETE;
run;

ods tagsets.ExcelXP file= "&results_dir./&round./benefittabs_newbs.xls" style=minimal options(orientation= 'landscape') ;
...
run;
ods tagsets.ExcelXP file= "&results_dir./&round./benefittabs_obs.xls" style=minimal options(orientation= 'landscape') ;
...
ods tagsets.ExcelXP file= "&results_dir./&round./benefittabs_AS.xls" style=minimal options(orientation= 'landscape') ;
